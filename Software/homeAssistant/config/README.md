# EV ESP32 Charging and energy monitor
please add two more items to the Home Assistant automations listed at the end.
...
Both items will be published to MQTT topic: ev-e-monitor/{{ charger_id }}/set where the payload will be a json doc key - value format.
...
- when ev-e-monitor/{{ charger_id }}/online becomes True
...
- id: ev_e_charging_sync
  ...
  trigger:
    ...
    # ESP comes online
    - platform: mqtt
      topic: ev-e-monitor/{{ charger_id }}/online
      payload: "True"
  ...
  action:
    ...
    - service: mqtt.publish
      data:
        topic: ev-e-monitor/{{ charger_id }}/set
        payload: >
